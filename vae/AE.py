...
class Autoencoder:
    ...
    def __init__(self, image_dim, conv_layer_specs, dense_layer_specs ,latent_dim, learning_rate=0.001, use_shared_weights=False):
        ...
        print("Flattened shape:", self.flattened_shape)
    ...
    # The autoencoder model
        self.create_model()
    # Global step for exponential learning rate decay
        self.global_step = tf.Variable(0)
        self.global_step_incr = tf.assign_add(self.global_step, 1)
    ...
    # Encoder
        self.z = self.encoder(self.x, enc_weights, enc_biases)
    ...
    def encoder(self, x, weights, biases):
        ...
        for i in range(len(self.dense_layer_specs)):
            ...
            hidden = tf.nn.relu(hidden)
        z_latent = tf.add(tf.matmul(hidden, weights['z']), biases['z'])
        return z_latent
    ...
    def fit(self, X):
        ...
        self.sess.run(self.global_step_incr)
        return cost
    ...
    def encode(self, X):
        mean = self.sess.run(self.z, feed_dict = {self.x: X})
        return mean