...
def make_critic(env, exp_data, hid_size=128, rnd_hid_size=128, reward_type=0, scale=250000, rnd_cnn_type=1):
    ...
    if reward_type == 0:
        critic = RND_Critic(ob_size, ac_size, hid_size=hid_size, rnd_hid_size=rnd_hid_size, scale=scale, 
                            rnd_hid_layer=2, hid_layer=4, rnd_cnn_type=rnd_cnn_type)
    elif reward_type == 1:
